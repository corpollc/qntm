...
The [AIM deployment runbook](aim-deploy.md) lists the Pages project, required token permissions, production-versus-preview behavior, live verification and rollback steps. Manual AIM runs always target production, even when their source ref is a feature branch.
...
The AIM browser build uses `npm ci` in tests and deployment. Its lockfile contains the platform-specific Rollup packages and patched dependencies within the declared version ranges. CI audits complete dependency graphs at moderate severity for the TypeScript library, AIM, TUI, relay/gateway/echo Workers, integration harness, OpenClaw and Claude channel, including development tools. NanoClaw remains a legacy adapter with its existing runtime-only check. Build the local TypeScript library before installing/building AIM, and publish only its static `dist/` directory at the domain root. No environment override selects a GitHub Pages subpath.

The three Worker packages override the development dependency `sharp` to 0.35.4 because Wrangler 4.130.0's Miniflare dependency pins 0.35.2. The [upstream security advisory](https://github.com/advisories/GHSA-rgj7-g3m4-5g8c) identifies 0.35.4 as the patched release. Keep the override until the upstream dependency is patched and the complete audit remains clear without it. It changes local emulation tooling; qntm does not bundle Sharp into its production Workers. Workers and the integration harness share the pinned Cloudflare 5.20260908.1 definitions; Vitest 3 users move to 4.1.11 with component and cross-surface coverage.
