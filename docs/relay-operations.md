...
Workers Paid was activated through the account checkout. The regular independent encrypted probe succeeded again at **19:07:48 UTC**. At approximately **19:11 UTC**, a separate disposable probe using published Python qntm 0.6.1 also verified authenticated live delivery and reconnect/replay from exe.dev. Aggregate telemetry was fresh, and Prometheus reported no active alerts. The checkout confirmation showed the paid subscription active; no relay deployment was needed for recovery.

Gateway idle-runtime reduction and account-level usage/budget monitoring remain separate follow-ups. A paid subscription includes metered overages, not a hard spending cap. No budget alert is claimed as configured by this incident record. No persistent Worker request logging was enabled during diagnosis.
