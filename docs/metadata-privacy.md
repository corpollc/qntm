...
The unreleased rate limiter rejects an untracked IP with HTTP 429 while its isolate's table is full; it does not evict active counters or reset existing quotas to admit newcomers. Existing IPs retain their remaining quota, and space becomes available when windows expire and another request triggers cleanup. Limits are per isolate, reset on isolate replacement, and do not apply to individual frames on an established WebSocket. They are not a global anti-abuse quota. Application cleanup does not delete any network metadata held separately by Cloudflare.
