...
The browser source shipped with 0.6.1, Python's `qntm convo invite` command and the terminal's `/invite` command generate `?invite=TOKEN` links: opening one sends its bootstrap secret to the browser UI host and any infrastructure that receives or logs that request URL. Removing it from the address bar afterward cannot undo this.

The hosted browser at `chat.corpo.llc` was updated on September 9, 2026 through [verified deployment 34409405374](https://github.com/corpollc/qntm/actions/runs/34409405374). Current browser code generates `#TOKEN` links; fragments are not sent in HTTP requests or Referer headers. The browser reads the fragment locally, opens a join review and removes it from the current address. This browser deployment did not create a new Python/npm release. The matching Python and terminal link-emission fixes are **unreleased**; use a current checkout or share the bare token until upgrading. TypeScript's `inviteToURL` and Python's new `invite_to_url` also discard old query parameters when building a link. Clipboard contents, browser history/sync, extensions, scripts on the UI origin and services used to share a link can still access it.
...
| In-memory rate limiter | Source IP string mapped to request `count` and `resetAt`. | Hosted relay: at most 10,000 IP entries per isolate, each key at most 64 characters. Fixed one-minute windows; every non-preflight request removes expired entries, including other idle IPs. No durable application IP table or IP metrics. With no subsequent request, expired entries can remain until isolate eviction; the counter window is not a guaranteed deletion timer. Relay source at the 0.6.1 tag has no cardinality bound or idle-entry purge. |
...
The bounded rate limiter was deployed to the hosted relay on September 9, 2026 through [verified deployment 34415183800](https://github.com/corpollc/qntm/actions/runs/34415183800), using commit `c5c4db2`. It rejects an untracked IP with HTTP 429 while its isolate's table is full; it does not evict active counters or reset existing quotas to admit newcomers. Existing IPs retain their remaining quota, and space becomes available when windows expire and another request triggers cleanup. Limits are per isolate, reset on isolate replacement, and do not apply to individual frames on an established WebSocket. They are not a global anti-abuse quota. Application cleanup does not delete any network metadata held separately by Cloudflare. This deployment did not change the published Python/npm package versions.
...
- Blackbox HTTPS/TLS results: fixed public target URLs, success, DNS/connect/TLS/HTTP durations, status and protocol, TLS certificate expiry and certificate identification exposed by the exporter. Targets are relay and charter health endpoints plus the root pages at `chat.corpo.llc` and `web.qntm.corpo.llc`. These describe synthetic endpoint checks, not user connections. Browser checks run once per minute per hostname, without profiles, cookies, invite tokens or conversation IDs; page bodies are not stored in metrics. Cloudflare still sees each probe's source IP, request headers, root URL and timing.
...
The optional OpenClaw gateway tool exposes complete reviews to the host and model, including request payloads, subjects, membership and policy. Credential plaintext is omitted from review output but is present in tool input, so the host transcript and configured provider can retain it. The wire credential is sealed to the accepted gateway. Gateway admission stores a private sealed bootstrap file with routing/identity metadata; it is removed after verified acceptance, otherwise retained until later admission overwrites it or the operator removes it. Review tokens and signed actions live in process memory for at most five minutes after preparation; they are not relay metrics or central approvals. See [tool privacy and retry behavior](../openclaw-qntm/README.md#optional-gateway-tools).
