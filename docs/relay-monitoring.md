...
The [23:18 UTC relay deployment](https://github.com/corpollc/qntm/actions/runs/34415183800) used commit `c5c4db2` and included the bounded in-memory IP rate limiter. All 13 verification jobs passed, and the resident collector reported a successful encrypted delivery/replay probe after rollout. Its Worker settings report returned `observability: null`, `logpush: false` and an empty Tail consumer list. Those settings do not establish account/zone export configuration or provider-internal retention; see the [metadata inventory](metadata-privacy.md).
