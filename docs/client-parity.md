...
Tests cover shared encrypted receive fixtures, Python 3.10/3.12, TypeScript crypto and subscriptions, the Python/TypeScript/Go charter boundary, browser/CLI/gateway journeys, terminal PTY input, and adapter contracts. The OpenClaw adapter also tests a compiled package installed in the real 2026.9.3 host, encrypted direct/group replies through a local relay fixture, rekey/removal across restart, and pending delivery recovery after host session-storage failure plus `SIGKILL`. A deterministic local model exercises the host's actual optional-tool loop for request, approval/withdrawal, credential and governance messages; ordinary chat transport uses a reply hook. These fixtures establish host compatibility, not agent judgment or universal harness support. OpenClaw uses the shared TypeScript authenticated reducer, private durable queue/checkpoint and opt-in reviewed gateway tools. Cross-client gateway execution/rejection acceptance remains open in `qntm-dhqb`. See [release checks](deployment-checklist.md), [OpenClaw gateway tools](../openclaw-qntm/README.md#optional-gateway-tools), [storage/recovery](../openclaw-qntm/README.md#local-storage-privacy-and-recovery), and [receive hooks](receive-hooks.md).
