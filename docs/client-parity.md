...
Tests cover shared encrypted receive fixtures, Python 3.10/3.12, TypeScript crypto and subscriptions, the Python/TypeScript/Go charter boundary, browser/CLI/gateway journeys, terminal PTY input, and adapter contracts. The OpenClaw adapter also tests a compiled package installed in the real 2026.9.3 host, encrypted direct/group replies through a local relay fixture, rekey/removal across restart, and pending delivery recovery after host session-storage failure plus `SIGKILL`. That test uses a deterministic reply hook and establishes host transport compatibility, not agent judgment or universal harness support. OpenClaw uses the same TypeScript authenticated reducer as the terminal, with its own private durable queue and checkpoint; structured outbound gateway actions remain incomplete. See [release checks](deployment-checklist.md), [OpenClaw storage/recovery](../openclaw-qntm/README.md#local-storage-privacy-and-recovery), and [receive hooks](receive-hooks.md).
