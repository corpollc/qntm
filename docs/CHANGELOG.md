...
- Added an optional OpenClaw gateway tool for requests, votes, credentials, governance and participant-initiated admission. Each action requires a complete prepare/commit review bound to the native host session and conversation, with fresh permission, membership and policy checks. Uncertain sends return a message ID without automatic retry; admission saves only sealed bootstrap for explicit retry and requires signed chat acceptance. Real OpenClaw 2026.9.3 agent-loop tests verify request/vote/credential/governance messages, alongside unit coverage of stale reviews and admission failures. Cross-client gateway execution/rejection acceptance remains open, so the README still marks this integration partial.
