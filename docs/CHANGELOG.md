...
- Acceptance tests now require successful HTTP health responses, bound stalled startup requests and report Worker output on failure. Fixtures isolate Wrangler discovery and discover OS-assigned Worker/browser listener ports, removing a race between selecting and binding a port. Restart checks retain the established endpoint, and shutdown cancels stale timers that could kill a replacement process. Failed setup no longer masks its diagnostics with a teardown error.
