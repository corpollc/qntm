...
- Added matching Python and TypeScript helpers that prepare an authorized contact addition with fresh group keys, recipient-encrypted welcomes and public group locator links. Tests cover earlier-history exclusion, removal/readmission, noncreator administrators, real relay delivery and fresh cross-language exchanges. Client interface integration and durable recovery remain unfinished; this library layer does not yet provide the complete add-contact workflow.
