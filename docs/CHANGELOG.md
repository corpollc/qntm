...
- Python resident receive waits for complete subscription replay before waking stdout or hooks, including after reconnect. Ordinary-group queues bind plaintext to verified ciphertext and invalidate superseded branch events, so a reused message ID cannot revive an old payload. Valid pending deliveries survive replay-cache eviction; a replacement welcome does not authorize previously queued plaintext. Replay buffer limits and local metadata are documented.
