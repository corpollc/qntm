...
func TestMultiAccountMessaging(t *testing.T) {
	...
	bobMessages1, err := dropboxMgr.ReceiveMessages(bobConversation, bobSeenMessages)
	if err != nil {
	...
	aliceMessages1, err := dropboxMgr.ReceiveMessages(aliceConversation, aliceSeenMessages)
	if err != nil {
		t.Fatalf("Alice failed to receive Bob's reply: %v", err)
	}
	
	if len(aliceMessages1) != 1 {
		t.Fatalf("Expected 1 reply message for Alice, got %d", len(aliceMessages1))
	}
	
	aliceRecvMsg1 := aliceMessages1[0]
	if string(aliceRecvMsg1.Inner.Body) != bobMessage1 {
		t.Fatalf("❌ Reply content mismatch!\n  Expected: %s\n  Got: %s", bobMessage1, string(aliceRecvMsg1.Inner.Body))
	}
	...
	for i, msg := range messages {
		...
		receivedMsgs, err := dropboxMgr.ReceiveMessages(receiverConv, *receiverSeenMsgs)
		if err != nil {
		...
		newMsg := receivedMsgs[len(receivedMsgs)-1]
		if string(newMsg.Inner.Body) != msg.text {
			t.Fatalf("❌ Message %d content mismatch!\n  Expected: %s\n  Got: %s", i+3, msg.text, string(newMsg.Inner.Body))
		}
