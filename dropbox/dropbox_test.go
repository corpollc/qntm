...
func TestManager_SendReceiveMessage(t *testing.T) {
	...
	messages, err := manager.ReceiveMessages(conversation, seenMessages)
	if err != nil {
	...
	// Verify message was deleted (ephemeral storage)
	exists, err = storage.Exists(key)
	if err != nil {
		t.Fatalf("Failed to check existence after receive: %v", err)
	}
	if exists {
		t.Error("Message should have been deleted after reading")
	}
}
...
func TestManager_ReplayProtection(t *testing.T) {
	...
	messages2, err := manager.ReceiveMessages(conversation, seenMessages)
	if err != nil {
...
func TestManager_WrongConversation(t *testing.T) {
	...
	_, conversation2 := createDropBoxTestSetup(t)
	...
	seenMessages := make(map[types.MessageID]bool)
	messages, err := manager.ReceiveMessages(conversation2, seenMessages)
	if err != nil {
		t.Fatalf("Failed to receive messages: %v", err)
	}
	...
	messages, err = manager.ReceiveMessages(conversation1, seenMessages)
	if err != nil {
