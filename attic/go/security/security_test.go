...
func TestPolicyEnforcer_CheckMessageSecurity_ReplayToggle(t *testing.T) {
	...
	envelope, err := messageMgr.CreateMessage(senderIdentity, conversation, "text", []byte("replay allowed"), nil, 3600)
	if err != nil {
		t.Fatalf("Failed to create test message: %v", err)
	}
	msg, err := messageMgr.DecryptMessage(envelope, conversation)
	...
	if err := enforcer.CheckMessageSecurity(envelope, msg.Inner, conversation); err != nil {
		t.Fatalf("First check should pass: %v", err)
	}
	if err := enforcer.CheckMessageSecurity(envelope, msg.Inner, conversation); err != nil {
		t.Fatalf("Second check should pass when replay protection is disabled: %v", err)
	}
	if enforcer.IsMessageSeen(envelope.ConvID, envelope.MsgID) {
		t.Fatal("message should not be tracked as seen when replay protection is disabled")
	}
}

func TestPolicyEnforcer_CheckMessageSecurity_ClockSkewToggle(t *testing.T) {
	conversation, senderIdentity := createSecurityTestSetup(t)
	messageMgr := message.NewManager()

	envelope, err := messageMgr.CreateMessage(senderIdentity, conversation, "text", []byte("clock skew test"), nil, 3600)
	if err != nil {
		t.Fatalf("Failed to create test message: %v", err)
	}
	msg, err := messageMgr.DecryptMessage(envelope, conversation)
	...
	tamperedEnvelope.CreatedTS = time.Now().Add(2 * time.Hour).Unix()
	tamperedEnvelope.ExpiryTS = tamperedEnvelope.CreatedTS + 3600

	withClockCfg := DefaultSecurityConfig()
	withClockCfg.EnableReplayProtection = false
	withClockCfg.EnableClockSkewCheck = true
	withClock := NewPolicyEnforcer(withClockCfg)

	err = withClock.CheckMessageSecurity(&tamperedEnvelope, msg.Inner, conversation)
	if err == nil || !strings.Contains(err.Error(), "clock skew check failed") {
		t.Fatalf("expected clock skew failure, got: %v", err)
	}

	noClockCfg := DefaultSecurityConfig()
	noClockCfg.EnableReplayProtection = false
	noClockCfg.EnableClockSkewCheck = false
	noClock := NewPolicyEnforcer(noClockCfg)

	err = noClock.CheckMessageSecurity(&tamperedEnvelope, msg.Inner, conversation)
	if err == nil || !strings.Contains(err.Error(), "signature check failed") {
		t.Fatalf("expected signature failure when clock check disabled, got: %v", err)
	}
}
