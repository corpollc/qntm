...
func TestEndToEndWorkflow(t *testing.T) {
	...
	messages, err := dropboxMgr.ReceiveMessages(bobConversation, seenMessages)
	if err != nil {
	...
	aliceMessages, err := dropboxMgr.ReceiveMessages(aliceConversation, aliceSeenMessages)
	if err != nil {
		t.Fatalf("Alice failed to receive Bob's reply: %v", err)
	}
	
	if len(aliceMessages) != 1 {
		t.Fatalf("Expected 1 reply message, got %d", len(aliceMessages))
	}
	
	reply := aliceMessages[0]
	if string(reply.Inner.Body) != bobReply {
		t.Errorf("Reply content mismatch: got %s, want %s", 
			string(reply.Inner.Body), bobReply)
	}
...
func TestGroupWorkflow(t *testing.T) {
	...
	bobMessages, err := dropboxMgr.ReceiveMessages(conversation, bobSeenMessages)
	if err != nil {
