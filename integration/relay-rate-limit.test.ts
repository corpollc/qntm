...
import { getFreePort, ManagedProcess, waitForHttp } from './src/runtime.js';
...
it('enforces HTTP 429 in the real Worker while leaving CORS preflight available', async () => {
  ...
  const relay = new ManagedProcess('rate-limit', [process.platform === 'win32' ? 'npx.cmd' : 'npx',
    ...
    '--persist-to', state, '--var', 'RATE_LIMIT_PER_MIN:2'], fileURLToPath(new URL('../worker', import.meta.url)), { ...process.env });
