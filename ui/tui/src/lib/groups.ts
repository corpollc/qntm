...
export class GroupBridge {
  ...
  private checkAvailable(): Promise<void> {
    if (!this.available) this.available = execute(this.executable(), ['-c',
      'import inspect; from qntm.group_client import GroupClient; from qntm.group_session import group_session_from_welcome; assert "challenge" in inspect.signature(GroupClient.refresh).parameters',
