...
function toCanonicalMap(obj) {
    ...
    if (typeof obj === 'object') {
        ...
        for (const key of keys) {
            map.set(key, toCanonicalMap(obj[key]));
        }
        return map;
    }
    return obj;
}
export function marshalCanonical(value) {
    const canonical = toCanonicalMap(value);
    return encoder.encode(canonical);
}
export function unmarshalCanonical(data) {
