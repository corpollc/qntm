...
describe('shared Ed25519 verification profile', () => {
  ...
  it('rechecks changed key bytes and verifies every message and signature after key validation', () => {
    const signer = generateIdentity(), other = generateIdentity(), suite = new QSP1Suite();
    const key = signer.publicKey.slice(), message = new TextEncoder().encode('current message');
    const signature = suite.sign(signer.privateKey, message);
    expect(isValidEd25519PublicKey(key)).toBe(true);
    expect(verifyEd25519Signature(key, message, signature)).toBe(true);
    key.fill(0); key[0] = 1;
    expect(isValidEd25519PublicKey(key)).toBe(false);
    expect(verifyEd25519Signature(key, message, signature)).toBe(false);
    key.set(other.publicKey);
    expect(isValidEd25519PublicKey(key)).toBe(true);
    expect(verifyEd25519Signature(key, message, signature)).toBe(false);
    key.set(signer.publicKey);
    expect(verifyEd25519Signature(key, message, signature)).toBe(true);
    expect(verifyEd25519Signature(key, new TextEncoder().encode('different message'), signature)).toBe(false);
    signature[63] ^= 0x80;
    expect(verifyEd25519Signature(key, message, signature)).toBe(false);
  });
  it('reuses public-byte validation but evicts old entries when peers supply many keys', () => {
    const keys = Array.from({ length: 257 }, () => generateIdentity().publicKey);
    const decode = vi.spyOn(ed25519.ExtendedPoint, 'fromHex');
    try {
      expect(isValidEd25519PublicKey(keys[0])).toBe(true);
      decode.mockClear();
      expect(isValidEd25519PublicKey(keys[0].slice())).toBe(true);
      expect(decode).not.toHaveBeenCalled();
      for (const key of keys.slice(1)) expect(isValidEd25519PublicKey(key)).toBe(true);
      decode.mockClear();
      expect(isValidEd25519PublicKey(keys[0])).toBe(true);
