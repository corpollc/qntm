...
export function keyIDFromPublicKey(pubkey: Uint8Array): Uint8Array {
  return suite.computeKeyID(pubkey);
}

export function verifyKeyID(pubkey: Uint8Array, keyID: Uint8Array): boolean {
  const computed = suite.computeKeyID(pubkey);
  return uint8ArrayEquals(computed, keyID);
}

export function serializeIdentity(identity: Identity): Uint8Array {
...
export function deserializeIdentity(data: Uint8Array): Identity {
  const obj = unmarshalCanonical<{ private_key: Uint8Array; public_key: Uint8Array; key_id: Uint8Array }>(data);
...
export function publicKeyToString(pubkey: Uint8Array): string {
...
export function keyIDToString(keyID: Uint8Array): string {
  return base64UrlEncode(keyID);
}

export function keyIDFromString(s: string): Uint8Array {
  const data = base64UrlDecode(s);
  if (data.length !== 16) {
...
export function generateConversationID(): Uint8Array {
